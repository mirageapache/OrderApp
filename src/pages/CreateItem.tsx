    ...
    <div id="create-item" className="create-item">
      ...
      {/* 餐點名稱 */}
        <label className="form-label" htmlFor="item-name">
          *餐點名稱
          <p className="err-msg" ref={nameMsgRef}>錯誤訊息!</p>
        </label>
        <input id="item-name" ref={nameRef} className="form-input" type="text" maxLength={50} placeholder="餐點名稱..." value={name} onChange={(e)=>{checkNameInput(e.target.value)}} />
      {/* 餐點說明 */}
        <label className="form-label" htmlFor="item-description">
          *餐點說明
          <p className="err-msg" ref={desMsgRef}>錯誤訊息!</p>
        </label>
        <textarea id="item-description" ref={desRef} className="form-input" cols={30} rows={3} maxLength={150} placeholder="敘述這個餐點的特色..." value={description} onChange={(e)=>{checkDesTextarea(e.target.value)}} ></textarea>
